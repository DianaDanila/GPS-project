...
namespace gps {
	...
	void Model3D::move(MOVE_DIRECTION direction, float speed, glm::vec3 &position)
	{
		switch (direction) {
	...
	glm::vec3 Model3D::getDir(){
		return this->objectDirection;
	}

	float Model3D::rotate(glm::vec3 obDir, glm::vec3 myDir){
		glm::vec3 objectDir = glm::vec3(obDir.x, 0.0f, obDir.z);
		glm::vec3 mineDir = glm::vec3(myDir.x, 0.0f, myDir.z);
		return glm::dot(glm::normalize(objectDir), glm::normalize(mineDir));
	}
}
